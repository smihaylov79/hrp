...
def generate_symbol_mapping():
    invest_map = {
        ...
        for item in DailyDataInvest.objects.filter(details__icontains='Stock').values('isin_number', 'symbol', 'company_name').distinct()
    }
    trade_map = {
        item['isin_number']: (item['symbol'], item['company_name'])
        for item in DailyData.objects.filter(details__icontains='Stock').values('isin_number', 'symbol', 'company_name').distinct()
    }
...
def fetch_fundamentals_data():
    symbols_mapping = SymbolsMapping.objects.all()
    today = date.today()
    errors = []
